...
The inference estimate volatility from the sequence itself.
...
TO DO: take into account coupled and uncoupled case -> make a new function
...
def posterior_volatility(seq, order, Nitem, theta_resol, vol_grid, vol_prior):
    ...
    # Initialize \theta, the statistics of observations
    ...
    post_nu = np.array(unnorm_log_post)
    post_nu = np.exp(post_nu) / sum(np.exp(post_nu))
